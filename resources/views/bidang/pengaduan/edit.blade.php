  ...
  <div class="row justify-content-center">
    <div class="col-lg-8">
      ...
      <div class="card shadow-sm border-0">
        <div class="card-header bg-light">
          <h5 class="mb-0">Hasil Telaah & Kirim ke Kepala BBWS</h5>
        </div>
        <div class="card-body">
          @if(session('success'))
          ...
          @endif
          <form action="{{ route('bidang.pengaduan.update', $report->id) }}" method="POST" enctype="multipart/form-data">
            @csrf @method('PUT')
            <div class="mb-3">
              <label class="form-label">Catatan Hasil Telaah (opsional)</label>
              <textarea name="notes" class="form-control" rows="3">{{ old('notes') }}</textarea>
            </div>
            <div class="mb-3">
              <label class="form-label">Upload Dokumen Output</label>
              <input type="file" name="file" class="form-control">
              <small class="text-muted">(.pdf/.jpg/.png — max 4MB)</small>
            </div>
            <div class="d-flex justify-content-end">
              <a href="{{ route('bidang.pengaduan.index') }}" class="btn btn-outline-secondary me-2">Batal</a>
              <button type="submit" class="btn btn-success">Kirim ke Kepala BBWS</button>
            </div>
          </form>
        </div>
      </div>
